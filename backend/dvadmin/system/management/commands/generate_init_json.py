...
class Command(BaseCommand):
    """
    القائمة المهيئة الإنتاج: python3 manage.py generate_init_json التهيئةmodelاسم
    على سبيل المثال：
    يولد：python3 manage.py generate_init_json
    توليد فقطmodelل： python3 manage.py generate_init_json users
    """
    ...
    def add_arguments(self, parser):
        parser.add_argument("generate_name", nargs="*", type=str, help="初始化生成ل表اسم")
    ...
    def handle(self, *args, **options):
        ...
        for generate_name in generate_name:
            if generate_name not in generate_name_dict:
                print(f"لم يتم تكوين طريقة التهيئة بعد\n{generate_name_dict}")
                raise Exception(f"لم يتم تكوين طريقة التهيئة بعد,عنصر التكوين:{list(generate_name_dict.keys())}")
