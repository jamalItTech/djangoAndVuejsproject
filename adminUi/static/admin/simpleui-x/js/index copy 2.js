...
,            
    ...
    assignEid(data) {
      ...
      iframeLoads(tab, e) {
        ...
        iframe.contentWindow.beforeLoad = () => {
          tab.loading = true;
          this.$forceUpdate();
        };
  
        const end = performance.now();
        console.log(`Iframe load time: ${end - start} ms`);
      },
      loadContentWithFetchs(url, tab) {
        const start = performance.now();
        fetch(url)
          .then(response => response.text())
          .then(html => {
            const end = performance.now();
            console.log(`Fetch load time: ${end - start} ms`);
            tab.content = html; // Set the HTML content for the tab
            tab.loading = false;
          })
    ...
    openTab(data, index, selected) {
      // تعيين نوع البيانات إلى 'fetch'
      data.type = "fetch";
    
      // تحقق مما إذا كانت التبويب هو لتحديث أو لا
      if (data.newTab) {
      ...
      // تحديث menuActive إذا تم تقديم index
      if (index) {
        this.menuActive = String(index);
      }
    
      // اختيار التبويب الحالي إذا كان موجودًا
      if (selected) {
        for (let i = 0; i < this.tabs.length; i++) {
          if (this.tabs[i].url === data.url) {
            this.tabModel = this.tabs[i].id;
      ...
      // التحقق مما إذا كانت التبويب موجودة
      let exists = this.tabs.find(tab => tab.eid === data.eid);
    
      if (exists) {
        // إذا كان التبويب موجودًا بالفعل، قم بالتبديل إليه
        this.tabModel = exists.id;
      } else {
        // تعيين حالة التحميل للتبويب الجديد
        data.id = data.eid;
        data.loading = true;
    
        // تنفيذ fetch لتحميل المحتوى
        const start = performance.now();
        
        fetch(data.url)
          .then(response => response.text())
          .then(html => {
            // قياس وتسجيل وقت تحميل fetch
            const end = performance.now();
            console.log(`Fetch load time: ${end - start} ms`);
    
            // تحديث محتوى التبويب وحالة التحميل
            data.content = html;
            data.loading = false;
    
            // إضافة التبويب الجديد إلى القائمة
            this.tabs.push(data);
            this.tabModel = data.id;
            this.$forceUpdate();
          })
          .catch(error => {
            ...
            data.loading = false; // تأكد من تحديث حالة التحميل في حالة الخطأ
            this.$forceUpdate();
          });
      }
    
      // مزامنة التبويبات وتغيير URL
      this.syncTabs();
      changeUrl(data);
    }
    ,
    ...
    loadContentWithFetch(url, tab) {
      ...
      fetch(url)
        ...
        .then(html => {
          const end = performance.now();
          console.log(`Fetch load time: ${end - start} ms`);
          let tabToUpdate = this.tabs.find(t => t.id === tab.id);
          if (tabToUpdate) {
            tabToUpdate.content = html; // Set the HTML content for the tab
